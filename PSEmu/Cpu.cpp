...
Cpu::Cpu(std::shared_ptr<MemoryMap> baseMemory): memory(baseMemory)
{
}
...
//			break;
//		default:
//			raise_exception(0x0a);// reserved instruction exception=
//			break;
//		}
//	default:
//		raise_exception(0x0a);// reserved instruction exception=
...
void Cpu::Clock()
{
	...
	*/
	...
	instruction o = next_instruction;
	instruction io{ 0 };
	io.address = program_counter;
	// todo:manage exception
	io.raw = memory->read32(program_counter);
	program_counter += 4;
	next_instruction = io;

	if (o.raw != 0) {
		// Decode(i-1)
		bool is_delay_slot = o.address & 1;
		o.address &= ~1;
		auto &rs = registers[o.p1];
		auto &rt = registers[o.p2];
		auto &rd = registers[o.p3];
		bool is_jump =
			o.code == 0 && (o.xcode > 1) == 4 || // jr/jalr
			o.code == 1 || // bltz/bgez/bltzal/bgezal
			o.code >> 2 == 1; //beq/bne/blez/bgtz
		bool is_mem = o.code >> 4 == 2;
		auto &target_reg = o.code == 0 ? rd : rt;
		auto target_addr = o.address + 4;

		// Execute(i-1)
		uint32_t return_value = 0;
		bool did_jump = false;
		bool save_return = false;
		bool exception_code = 0;
		uint32_t jump_addr = 0;
		if (!is_mem) {
			auto result = AluOpcodes::alu_dispatch(o, rs, rt);
			did_jump = !!(result.exc & 1);
			save_return = !!(result.exc & 2);
			exception_code = !!(result.exc & 0x7c);
			return_value = result.res;
		}
		//check exception/jump
		if (exception_code) {
			program_counter = coprocessor.raise_exception(o.address|(is_delay_slot?1:0), exception_code >> 2, 0);
			next_instruction = { 0 };
		...
		// Memory (i-1)
		if (is_mem) {
			return_value = UpdateMemory(o, rs, rt);
		}
